    ...
    public function register_account_admin()
    {
        try {
            ...
            if ($this->Password != $this->ConfirmPassword || strlen($this->Password) < 8) {
                ...
                try {
                    $Query = connection_database()->query($insert_query);
                    if (!$Query) {
    ...
    public function login_admin_account()
    {
        ...
        try {
            if (empty($this->UserName_Email) || empty($this->Password)) {
                ...
                try {
                    $result = connection_database()->query($selectQuery);
                    if (empty($result)) {
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        if (!empty($_POST)) {
            ...
            $admin_register = new admin_signin_account($UserName, $Email, $Password, $ConfirmPassword, $Gender, $UserName_Email);
        }
